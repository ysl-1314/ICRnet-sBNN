...
class MaxMinPointDist(torch.nn.Module):
    ...
    def forward(self, point):
        ...
        point1 = point.unsqueeze(0).unsqueeze(0).repeat(1, self.point_num, 1, 1).float()
        point2 = point.unsqueeze(0).unsqueeze(2).repeat(1, 1, self.point_num, 1).float()

        dist = torch.norm(point1 - point2, dim=3) + self.bigeye

        min_dist, _ = torch.min(dist, dim=2)

        return torch.max(min_dist, dim=1)[0]